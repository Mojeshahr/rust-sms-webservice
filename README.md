...
زبان Rust نه کلاینت HTTP در کتابخانه استانداردش دارد و نه JSON، پس برخلاف نمونه‌های
PHP و Python و Node و C# نمی‌شود صفر وابستگی داشت. یکی دارد.

کتابخانه `ureq` با ویژگی `json` هر دو را می‌دهد، چون خودش `serde_json` را دوباره صادر
...
**کتابخانه `ureq` وضعیت غیر ۲xx را خودش خطا حساب می‌کند.** یعنی نشانی اشتباه به‌جای
...
سؤال یا خطایی هست؟ [issue باز کنید](https://github.com/Mojeshahr/rust-sms-webservice/issues)
